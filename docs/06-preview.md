...
- **反映方式** … 設定を変えたときの反映のしかたです
  ...
  スキャンPDFのような重い原稿では、1回の作り直しに数秒かかることがあります。プレビューの範囲を狭くするか、反映方式を「手動」にすると軽くなります（→ [13章](13-image-correction.md#濃淡補正の調整が重いとき)）。
- **プレビュー 開始 ～ 終了** … プレビューするページの範囲です。
  開始・終了とも1〜65,535ページを指定でき、既定は1〜20ページです。
  長い原稿の後半だけを確かめたいときに、後ろのページを直接指定できます。
  PDF・ZIP/CBZ・画像フォルダは、開始ページより前の画像処理を省くため、後ろのページを
  指定しても遅くなりません（開始・終了に同じ番号を入れれば、その1ページだけを確認できます）。
  テキスト・EPUB・Markdownは、前のページの内容が組版に影響するため、これまでどおり
  先頭から組みます
