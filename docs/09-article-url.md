...
通常の記事に加え、JavaScriptで表示されるページ、Xの投稿・同一投稿者のスレッド、
ログイン後に自分が閲覧できる購入済み記事を取り込めます。

> 閲覧権限のない有料部分を迂回する機能ではありません。

取得したあとで組版の設定（プリセットや文字サイズなど）を変えたときは、
**［設定をプレビューに反映］** を押すと、ページを取り直さずに組み直せます。
...
取り込んだ記事は記事データ（`.xtca`）として保存できます。
歯車メニューの **記事データを開く…** から選ぶと、もう一度変換対象として開けます。
