...
- ページサイズは **480×800px**（Xteink X4の実解像度）。Webページ上では縮小表示しています
- **画像をクリックすると原寸で開きます**
...
![TXTを縦書き・明朝体で変換した出力](../images/sample-output-txt-mincho.png)

フォントを明朝体（NotoSerifJP）に変えただけで、それ以外は上と同じ設定です。
**小説はこちらのほうが読みやすいと感じる方が多い**はずです。フォントは
[組版ブロック](03-typesetting.md#フォントサイズ余白)の「参照…」から変更します。
...
| 項目 | 既定 | 煮詰めた例 | ねらい |
|---|---|---|---|
| フォント | NotoSansJP-SemiBold（ゴシック） | NotoSerifJP（明朝） | 長文の縦組みは明朝のほうが目が滑りにくい |
| 本文サイズ | 26 | 24 | わずかに小さくして1行の情報量を確保 |
| 行間 | 44 | 43 | 文字を小さくした分、相対的に行間が広くなり風通しがよくなる |
| 余白（上/下/左右） | 12 / 14 / 12 | 18 / 22 / 18 | 天地の詰まりを解消し、本らしい佇まいにする |

いずれも [組版ブロック](03-typesetting.md#フォントサイズ余白) で設定できます。
