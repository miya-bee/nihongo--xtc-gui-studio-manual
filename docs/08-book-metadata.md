...
> **同じページから始まる見出しは、変換時に1つの章へまとめられます。**
> この統合はページ位置で決まるためプレビューでは再現できず、短い文書では
> 一覧に並ぶ数より実際の章が少なくなることがあります（タブ内にも注記が出ます）。
> 章のタイトル自体は、一覧と実際に埋め込まれるものが一致します。
...
書籍情報ダイアログは、変換対象を選んだ状態で歯車メニューの **書籍情報…** から開きます。
ファイルを開いたときに「書籍情報を開きますか？」と聞かれた場合も、ここへ進みます。
[XTCツール](14-xtc-tools.md)からも、既存のXTC/XTCHに対して同じ編集ができます。

書籍情報を確定する（OK）と、メイン画面のプレビューが作り直され、表紙や書名の変更がすぐ反映されます。
キャンセルしたときは作り直しません。
